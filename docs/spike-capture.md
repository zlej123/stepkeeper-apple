...
```bash
STEPKIPPER_SPIKE=1 ./build/Build/Products/Debug/stepkipper.app/Contents/MacOS/stepkipper &
```
...
1. `ps eww -p <pid>`로 확인 — `STEPKIPPER_SPIKE=1`이 프로세스 환경에 정확히 전달됨을 확인
   (환경변수 전달 자체는 문제 아님).
...
3. TCC 관련 가능성도 배제하지 않고 확인 시도 — `tccd` 유닛 로그에서 `com.stepkipper.app`에
...
4. **해결**: `launchctl setenv STEPKIPPER_SPIKE 1` + `open build/.../stepkipper.app`(LaunchServices
...
브리프 Step 10 문서의 원문 커맨드(`STEPKIPPER_SPIKE=1 ./path/to/binary &`)는 이 환경에서는
...
`~/Library/Containers/com.stepkipper.app/Data/Documents/spike/result.json` /
...
launchctl setenv STEPKIPPER_SPIKE 1
open build/Build/Products/Debug/stepkipper.app
# 결과: ~/Library/Caches/spike/result.json (최대 수십초 대기, 광고가 끼면 최악 수 분)
launchctl unsetenv STEPKIPPER_SPIKE   # 정리
pkill -x stepkipper                   # 정리
```

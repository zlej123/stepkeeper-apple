...
사용: python3 scripts/make-golden.py   (코어 위치는 STEPKIPPER_PATH, 기본 ../stepkeeper)
...
from apple_brand import apply_brand
...
for case_dir in sorted(p for p in golden_root.iterdir() if p.is_dir()):
    ...
    markdown = apply_brand(core_render.render(body, context)).strip() + "\n"

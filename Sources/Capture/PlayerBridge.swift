...
final class PlayerBridge: NSObject, ObservableObject {
    ...
    func waitForMetadata(expecting videoID: String? = nil,
        ...
        while Date() < deadline {
            ...
            let result = try? await callJS(
                "if (!window.__stepkipper) { return null; } return JSON.stringify(await window.__stepkipper.waitMeta(1500, \(expectId)));",
                timeout: 5)
            if let json = result,
    ...
    func primePlayer() async throws {
        ...
        _ = try await callJS("await window.__stepkipper.prime(); return \"ok\";", timeout: 10)
    }
    ...
    func probePageState() async throws -> String {
        ...
        return location.href + " | bridge=" + typeof window.__stepkipper + " | vis=" + document.visibilityState + " | playerState=" + (mp && mp.getPlayerState ? mp.getPlayerState() : "n/a") + " | video=" + (v ? ("w=" + v.videoWidth + " rs=" + v.readyState + " paused=" + v.paused + " buf=" + buf + " dur=" + v.duration) : "none") + " | title=" + document.title;
        """
    ...
    func captureFrame(at seconds: Int) async throws -> Data {
        ...
        do {
            ...
            result = try await callJS(
                "return await window.__stepkipper.capture(\(seconds), 8000);", timeout: 30)
        } catch {
    ...
    func beginCaptureSession() async throws {
        do {
            _ = try await callJS("return await window.__stepkipper.captureBegin();", timeout: 8)
        } catch {
    ...
    func endCaptureSession() async {
        _ = try? await callJS("return await window.__stepkipper.captureEnd();", timeout: 5)
    }
}
